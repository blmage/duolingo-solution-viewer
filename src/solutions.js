 ...
 * @param {object} metaData The meta data from the corresponding challenge.
 * @param {string} locale The locale of the translations.
 * @return {Solution[]} The corresponding set of solutions.
 */
export const fromCompactTranslations = (translations, metaData, locale) => {
  ...
  if ('ja' === locale) {
    ...
    const translationGroups = Object.values(
      groupBy(
        preparedTranslations.flatMap(original => converters.map(it(original))),
        it.sentence
      )
    );

    preparedTranslations = translationGroups.map(group => ({
  ...
  for (const { sentence, flags } of preparedTranslations) {
    ...
    for (const choiceSet of choiceSets) {
      ...
      for (const [ lengthKey, subChoices ] of Object.entries(choices)) {
...
const getPatternSolution = (pattern, solutionIx, locale) => {
  ...
  for (const branchSet of pattern.branchSets) {
    for (let i = position; i < branchSet.tokenPosition; i++) {
      tokens.push([ pattern.sharedTokens[i] ]);
      reference += pattern.sharedTokens[i];
    }
    ...
    if (branch.firstChoice.length > 0) {
      isComplex = true;
      tokens.push(branch.firstChoice);
      reference += branch.firstChoice[0];
    }

    if (branch.sharedTokens.length > 0) {
      tokens.push(...branch.sharedTokens.map([ it ]));
      reference += branch.sharedTokens.join('');
    }

    if (branch.lastChoice.length > 0) {
      isComplex = true;
      tokens.push(branch.lastChoice);
      reference += branch.lastChoice[0];
    }
